...
import {
  Play, ArrowLeft, Plus, Trash2, Download, Film, Tv, Loader2, LogOut, Menu,
  Image as ImageIcon, FileText, Clapperboard, Swords, Sparkles, Link2, X
} from 'lucide-react'
        ...
        >
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <p className="text-white/30 text-sm">Add new movies, series, or anime to your library</p>
            </div>
          </div>
        </motion.div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <CardContent className="space-y-5">
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {([
                      { value: 'Movie' as ContentType, icon: Film, label: 'Movie', emoji: '🎬', activeClass: 'bg-blue-500/15 text-blue-400 border-blue-500/40 shadow-lg shadow-blue-500/10' },
                      { value: 'Series' as ContentType, icon: Tv, label: 'Series', emoji: '📺', activeClass: 'bg-emerald-500/15 text-emerald-400 border-emerald-500/40 shadow-lg shadow-emerald-500/10' },
                      { value: 'Anime' as ContentType, icon: Swords, label: 'Anime', emoji: '⚔️', activeClass: 'bg-orange-500/15 text-orange-400 border-orange-500/40 shadow-lg shadow-orange-500/10' },
                    ]).map(type => (
                      <button
                        key={type.value}
                        type="button"
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-white/20 text-xs">Paste the HTML img code for the content image.</p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-start gap-4">
                    <div className="w-40 sm:w-48 aspect-[2/3] rounded-xl overflow-hidden bg-[#1a1a25]/50 border border-white/[0.06] flex items-center justify-center">
                      {imageSrc ? (
                    ...
                    <div className="flex-1 hidden sm:block">
                      <div className="bg-[#1a1a25]/50 rounded-xl p-4 border border-white/[0.06]">
              ...
              <CardContent>
                {downloadGroups.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-12 text-center">
                    ...
                    <p className="text-white/30 text-sm mb-1 font-medium">No download groups added yet</p>
                    <p className="text-white/15 text-xs">Click the button above to add a download group</p>
                  </div>
                ) : (
                        ...
                        >
                          ...
                          <div className="flex items-center justify-between mb-4">
                            <div className="flex items-center gap-2.5">
                              ...
                              <span className="text-white/50 text-sm font-medium">Download Group {gi + 1}</span>
                            </div>
                            <button type="button" onClick={() => confirmDeleteGroup(group.id)} className="h-8 w-8 flex items-center justify-center text-white/20 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-colors">
                                  ...
                                  {group.links.map((link) => (
                                    ...
                                    >
                                        ...
                                        <div className="flex flex-wrap gap-1.5">
                                          {QUALITY_OPTIONS.map((q) => (
                                            <button key={q} type="button" onClick={() => {
                                              ...
                                              className={`px-2.5 py-1 rounded-lg text-[10px] font-bold transition-all duration-200 border uppercase ${
                                                link.quality.includes(q) ? qualityColorMap[q] : `bg-[#0a0a0f]/80 ${qualityBorderMap[q]} border hover:opacity-80`
                                              }`}>
                                              {q}
                                            </button>
                                          ))}
                                      ...
                                      <div className="flex justify-end">
                                        <button type="button" onClick={() => removeDownloadLink(group.id, link.id)} className="text-xs text-red-400/40 hover:text-red-400 hover:bg-red-500/10 px-2 py-1 rounded-lg flex items-center gap-1 transition-colors">
          ...
          >
            ...
            >
              ...
              {submitting ? (
                <span className="flex items-center gap-2 relative z-10"><Loader2 className="h-5 w-5 animate-spin" />Creating...</span>
              ) : (
                <span className="flex items-center gap-2 relative z-10"><Plus className="h-5 w-5" />Create Content</span>
              )}
            </Button>
